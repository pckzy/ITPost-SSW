...
function deletePost() {
    if (postToDelete) {
        ...
        fetch(`/post/${postId}/delete/`, { method: 'POST', headers: headers })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
...
function toggleAllPost() {
    const allpost_div = document.getElementById('allpost-div')
    if (allpost_div.classList.contains('max-h-0')) {
        allpost_div.classList.remove('max-h-0')
        allpost_div.classList.add('max-h-[1500px]')
    } else {
        allpost_div.classList.add('max-h-0')
        allpost_div.classList.remove('max-h-[1500px]')
    }
}